...
# Phase 1: LLM imports
from .llm import (
    ILLMService, OllamaClient, ModelRouter, TaskType, LLMConfig,
    LLMProviderConfig, ModelRoutingConfig, PromptTemplateManager,
    get_template_manager, render_prompt, LLMError
)
...
class AIExtension(IExtension):
    ...
    async def initialize(self, container: ServiceContainer) -> None:
        ...
        try:
            ...
            raise ExtensionInitializationError(
                f"AI extension initialization failed: {str(e)}", 
                extension_name="ai",
                cause=e
            )
    ...
    async def _handle_ai_test(self, arguments: Dict[str, Any]) -> List[types.TextContent]:
        ...
        if self.container:
            response += f"- Container Available: True\n"
            response += f"- Settings Available: {hasattr(self.container, 'settings')}\n"
        else:
            response += f"- Container Available: False\n"
    ...
    async def _handle_llm_quick_answer(self, arguments: Dict[str, Any]) -> List[types.TextContent]:
        ...
        try:
            ...
            error_msg = f"LLM quick answer failed: {str(e)}"
            return [types.TextContent(type="text", text=error_msg)]
