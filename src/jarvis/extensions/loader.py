...
from jarvis.extensions.interfaces import IExtension, IExtensionManager, ExtensionMetadata, ExtensionStatus, ExtensionHealth
from jarvis.extensions.errors import (
    ExtensionError, ExtensionLoadError, ExtensionInitializationError,
    ExtensionNotFoundError, ExtensionAlreadyLoadedError, ExtensionDependencyError
)
...
class ExtensionLoader(IExtensionManager):
        ...
        async with self._loading_lock:
            ...
            try:
                ...
                if isinstance(e, ExtensionError):
                    raise
                raise ExtensionLoadError(f"Failed to load extension {extension_name}: {str(e)}", extension_name, e)
    ...
    def _find_extension_path(self, extension_name: str) -> Optional[Path]:
